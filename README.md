...
Dataset :  The [SST2 dataset](https://huggingface.co/datasets/sst2) which was used for the sentiment analysis.

Model : We will be using the [BERT base model (uncased)](https://huggingface.co/bert-base-uncased).

Hardware Description : the code which was used for this model !nvidia-smi

![](https://lh7-us.googleusercontent.com/SqGvaqiyokpnp0xGpkrBtCL6UtquubkMLVrIp7FOh64mW_1gUHiGIhZBWQpNxZThxP_HvaCuxholCDtZOq_-mL4aIELuMA2iQtffz2IwYyCUOOxfdQSRMV-TC5ETiNf-U7TBYliyN837AblhdaB5CyI)

### References:

1.  References to Hugging Face Libraries:
...
1.  ### Reference to Hugging Face Tutorial on Fine-Tuning :
...
![](https://lh7-us.googleusercontent.com/hWe0FM_BsST_c60BH607Xa1XSK8Bt4KOGBsFBLTToRo_D3gUJjkUQ-PtoUfXAR7qtxzLIb8iZLBnRSqB-kikqpVaXiOfnmWQy0DvcLynrOqpKJmIQKpupDwts9vmMlmxYVFCpaUhOaS54ExK6dphulI)
...
1.  Model Configuration: Differences in the specific configuration

2.  Data Preparation: There could be variations in how the dataset is pre-processed.

3.  Regularization Techniques: The original benchmark might be using a more sophisticated benchmark.

Ways to Improve the model performance:

1.  Hyperparameter Optimization: Trying out different settings for the learning rate and a different batch size and the number of training epochs.

2.  Better Regularization: We could also integrate dropout, label smoothing etc to augment the training data.

3.  Review Training Process: The training process closely aligns with the benchmark, including any specific training techniques.

This is the Training Loss Vs the Validation Loss. 

![](https://lh7-us.googleusercontent.com/ngj8qUHos1EWFjzumNJKz8wFjQshaKFn4S7-xF9EiO7s2IDCOb12cwLhEUvcHUbLbxrARa1Sis55vvCFN_ErxuZfpj0Kil5bZzEfptkjgSq1aKvfSPSCWhq1wcyvGpFWgADKKjIEZFecU-WOf5PpdD8)
...
1.  Improved Relations Of Negation and Sentiment:
...
1.  Model Architectural Adjustments and Hybrid Models:
